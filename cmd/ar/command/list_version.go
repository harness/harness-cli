...
// newListVersionCmd wires up:
//
//	hns ar version list
func NewListVersionCmd() *cobra.Command {
	var host string
	var format string
	cmd := &cobra.Command{
		Use:   "version list",
		Short: "List all versions of an artifact",
		Long:  "Lists all available versions of a specific artifact in the Harness Artifact Registry",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Call API
			//resp, err := cli.GetAllArtifactVersions(context.Background())
			//if err != nil {
			//	return err
			//}
