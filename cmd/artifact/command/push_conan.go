...
const (
	...
	conanFileConanfilePy  = "conanfile.py"  // recipe layer
	conanFileConaninfoTxt = "conaninfo.txt" // package layer
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, filePath := range orderConanFiles(recipeFiles) {
				if err := uploadConanRecipeFile(pkgClient, registryName, ref, recipeRevision, filePath, progress); err != nil {
					return err
				}
			}
			...
			if packageDir != "" {
				...
				for _, filePath := range orderConanFiles(packageFiles) {
					if err := uploadConanPackageFile(pkgClient, registryName, ref, recipeRevision, packageID,
...
func isValidConanRecipeFile(name string) bool {
	if name == conanFileConanfilePy || name == conanManifestFile {
		return true
	}
...
func isValidConanPackageFile(name string) bool {
	if name == conanFileConaninfoTxt || name == conanManifestFile {
		return true
	}
	prefix, ok := conanTarballPrefix(name)
...
func uploadConanRecipeFile(
	client *pkgclient.ClientWithResponses,
	registryName string,
	ref conanReference,
	rrev string,
	filePath string,
	...
	file, checksums, err := openConanFile(filePath, progress)
	if err != nil {
		return err
	}
	defer file.Close()
...
// openConanFile validates the file, computes its checksums, and returns an open handle.
func openConanFile(filePath string, progress *p.ConsoleReporter) (*os.File, utils.FileChecksums, error) {
	info, err := os.Stat(filePath)
	if err != nil {
		return nil, utils.FileChecksums{}, errors.NewValidationError("file_path", fmt.Sprintf("failed to access file: %v", err))
	}
	if info.IsDir() {
		return nil, utils.FileChecksums{}, errors.NewValidationError("file_path", fmt.Sprintf("expected a file, got a directory: %s", filePath))
	}

	checksums, err := utils.ComputeFileChecksums(filePath)
	if err != nil {
		progress.Error("Failed to compute file checksums")
		return nil, utils.FileChecksums{}, fmt.Errorf("failed to compute checksums for %s: %w", filePath, err)
	}

	file, err := os.Open(filePath)
	if err != nil {
		progress.Error("Failed to open file")
		return nil, utils.FileChecksums{}, err
	}
	return file, checksums, nil
}
