...
func RunFirewallExplain(f *cmdutils.Factory, registryUUID uuid.UUID, artifacts []ar_v3.ArtifactScanInput, org, project string, progress p.Reporter) error {
	if len(artifacts) == 0 {
		return nil
	}
	...
	for batchIdx := 0; batchIdx < totalBatches; batchIdx++ {
		...
		if err != nil {
			return fmt.Errorf("evaluation failed after %d attempts: %w", maxRetries, err)
		}
		allScans = append(allScans, scans...)
	}

	progress.Success(fmt.Sprintf("Firewall evaluation completed (%d packages)", len(allScans)))
	return DisplayBlockedScanResults(f, allScans, progress)
}
