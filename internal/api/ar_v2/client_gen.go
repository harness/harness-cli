...
type MetadataInput struct {
	// ArtifactKeyFilters Optional artifact key filters for Debian packages.
	// Key-value pairs for artifact identification.
	// Example: {"architecture": "amd64", "distribution": "focal", "component": "main"}
	ArtifactKeyFilters *map[string]string `json:"artifactKeyFilters,omitempty"`
...
// FiltersParam defines model for FiltersParam.
type FiltersParam = []string
...
type CopyRegistryPackageParams struct {
	...
	// Filters Key-value filters for artifact retrieval. Repeat the parameter once per filter.
	// Format per entry: key:value
	// Example: ?filters=architecture:amd64&filters=distribution:jammy
	Filters *FiltersParam `form:"filters,omitempty" json:"filters,omitempty"`
}
