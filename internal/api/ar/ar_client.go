...
// ARClient represents the client for Harness Artifact Registry API
...
func NewHARClient(baseURL string, authToken string, accountID string, orgID string, projectID string) *ARClient {
	return &ARClient{
		BaseURL:    baseURL,
...
// RegistryRequest represents a request to create or update a registry
...
// RegistryResponse represents the API response for registry operations
...
func (c *ARClient) CreateRegistry(req RegistryRequest) (*RegistryResponse, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("error marshaling request: %w", err)
	}
	endpoint := fmt.Sprintf("%s?space_ref=%s", "/registry", api.GetRef(c.AccountID, c.OrgID, c.ProjectID))
...
// GetRegistry retrieves details of a registry
func (c *ARClient) GetRegistry(registryRef string) (*RegistryResponse, error) {
	endpoint := fmt.Sprintf("/registry/%s", url.PathEscape(registryRef))
...
func (c *ARClient) UpdateRegistry(registryRef string, req RegistryRequest) (*RegistryResponse, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("error marshaling request: %w", err)
	}
...
func (c *ARClient) DeleteRegistry(registryRef string) error {
	endpoint := fmt.Sprintf("/registry/%s", url.PathEscape(registryRef))
...
func (c *ARClient) ListRegistries(
	packageTypes []string,
	registryType string,
	page, size int,
	searchTerm string,
	...
	var endpoint string
	endpoint = fmt.Sprintf("/spaces/%s/+/registries", api.GetRef(c.AccountID, c.OrgID, c.ProjectID))
...
// ListArtifacts lists artifacts in a registry
func (c *ARClient) ListArtifacts(
	registryRef string,
	labels []string,
	page, size int,
	searchTerm string,
) (*ListArtifactsResponse, error) {
	endpoint := fmt.Sprintf("/registry/%s/+/artifacts", registryRef)
...
func (c *ARClient) GetArtifactDetail(registryRef, artifact, version string) (*ArtifactDetailResponse, error) {
	endpoint := fmt.Sprintf("/registry/%s/artifact/%s/version/%s/details",
...
func (c *ARClient) DeleteArtifactVersion(registryRef, artifact, version string) error {
	endpoint := fmt.Sprintf("/registry/%s/artifact/%s/version/%s",
		url.PathEscape(registryRef), url.PathEscape(artifact), url.PathEscape(version))
...
func (c *ARClient) UpdateArtifactLabels(registryRef, artifact string, labels []string) (
	*ArtifactLabelResponse,
	...
	endpoint := fmt.Sprintf("/registry/%s/artifact/%s/labels",
...
func (c *ARClient) GetArtifactSummary(registryRef, artifact string) (*ArtifactSummaryResponse, error) {
	endpoint := fmt.Sprintf("/registry/%s/artifact/%s/summary",
		url.PathEscape(registryRef), url.PathEscape(artifact))
...
// GetClientSetupDetails gets client setup details for a registry
func (c *ARClient) GetClientSetupDetails(registryRef string, artifact, version string) (*ClientSetupResponse, error) {
	endpoint := fmt.Sprintf("/registry/%s/client-setup-details", url.PathEscape(registryRef))
...
func (c *ARClient) doRequest(method, path string, body []byte) (*http.Response, error) {
...
func (c *ARClient) StartMigration(req MigrationRequest) (string, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return "", fmt.Errorf("error marshaling request: %w", err)
	}

	resp, err := c.doRequest("POST", "/api/v1/migration/start", body)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
...
func (c *ARClient) GetMigrationStatus(migrationID string) (*MigrationStatus, error) {
	resp, err := c.doRequest("GET", fmt.Sprintf("/api/v1/migration/%s/status", migrationID), nil)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
...
func (c *ARClient) UpdateArtifactStatus(migrationID string, req ArtifactUpdateRequest) error {
	body, err := json.Marshal(req)
	if err != nil {
		return fmt.Errorf("error marshaling request: %w", err)
	}

	resp, err := c.doRequest("PUT", fmt.Sprintf("/api/v1/migration/%s/update", migrationID), body)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
// GetArtifacts retrieves all artifacts in a registry
func (c *ARClient) GetArtifacts(registryID string) ([]map[string]interface{}, error) {
	resp, err := c.doRequest("GET", fmt.Sprintf("/api/v1/migration/artifacts/%s", registryID), nil)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
