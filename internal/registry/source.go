...
// NewJFrogSourceRegistry creates a new JFrog source registry client
func NewJFrogSourceRegistry(cfg config.SourceConfig) (*JFrogSourceRegistry, error) {
	if cfg.Type != "JFROG" {
...
// ListArtifacts lists all artifacts in a JFrog registry
func (j *JFrogSourceRegistry) ListArtifacts(registry string) ([]Artifact, error) {
	// Check if the requested registry is in the filtered registries
	registryFound := false
...
func (j *JFrogSourceRegistry) executeAQLQuery(query map[string]interface{}) ([]Artifact, error) {
	...
	aqlURL := *baseURL
	aqlURL.Path = path.Join(aqlURL.Path, "api/search/aql")
