...
// NewHARDestinationRegistry creates a new HAR destination registry client
func NewHARDestinationRegistry(cfg config.DestinationConfig) (*HARDestinationRegistry, error) {
	if cfg.Type != "HAR" {
...
func (h *HARDestinationRegistry) UploadArtifact(artifact Artifact, data []byte) error {
	// Get base URL for registry endpoint
	baseURL, err := url.Parse(h.config.RegistryEndpoint)
	if err != nil {
		return fmt.Errorf("invalid registry endpoint URL: %w", err)
	}
	...
	if len(registryParts) == 0 {
		return fmt.Errorf("invalid destination registry format: %s", artifact.Registry)
	}
...
// CreateRegistry creates a new registry in HAR
func (h *HARDestinationRegistry) CreateRegistry(registry string) error {
	...
	// Parse registry path to extract org and project IDs if present
	// Format could be: "registry", "org/registry", or "org/project/registry"
	parts := strings.Split(registry, "/")

	createRequest := map[string]interface{}{
		"name":              parts[len(parts)-1], // Last part is always the registry name
		"type":              "generic",           // Default type, can be overridden based on artifact types
		"accountIdentifier": h.config.AccountIdentifier,
	}
	...
	if err != nil {
		return fmt.Errorf("error marshaling create registry request: %w", err)
	}

	// Construct the create registry URL
	createURL := *baseURL
	createURL.Path = path.Join(createURL.Path, "api/registries")
	...
	// 409 Conflict typically means the registry already exists
